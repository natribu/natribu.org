...
$memcache->set("count_na_$lang",$count,600); // Á‡ÔËÒ‡Ú¸ ‚ memcache
...
if ($lju && ($userdata[1]=="")) {
	...
	echo "</font>";
	//if ($lju == "doctor-livsy") { echo "<br>‰ÂÎ‡Ú¸ ‚‡Ï ÌÂ˜Â„Ó, ‰ÓÍÚÓ :)"; }
...
function poiskovik () {
	...
	if (ereg ("rambler.ru", $u["host"])) {
		...
		$k_koi=strlen(str_replace("-","",strtr($searchtext," √’À≈Œ«€›⁄»ﬂ∆Ÿ◊¡–“œÃƒ÷‹—ﬁ”Õ…‘ÿ¬¿","--------------------------------")));
		$k_win=strlen(str_replace("-","",strtr($searchtext,"Í„ıÎÂÓÁ˚˝˙ËˇÊ˘˜·ÚÔÏ‰ˆ¸Ò˛ÛÌÈÙ¯‚‡","--------------------------------")));
