...
if (strlen($_POST['tolang']) > 3) { print "<p>error: newerland :)"; exit(); }
...
if (file_exists("lang/".$tolang.".php")) {
    ...
    if ($_POST['password']!=$l2['password']) { print "error password: ".$_POST['password']; exit(); }
    }

    if ($l2['changed']!="web") { echo "<p>backup...<p>".$l2['changed']; rename ("lang/".$tolang.".php","lang/".$tolang.".php.old"); }
...
if (
    (strlen($tolang) > 3) ||
    (strlen($tolang) < 2) ) { print "<p>error: newerland :)"; exit(); }
...
if ($l2['codepage']) { $codepage=$l2['codepage']; }
...
$i1=0;
foreach ($masstr as $t) { $i1++; if (strlen($t) >= $cols) $i1=$i1+(floor(strlen($t) / $cols)); }

//$masstr=split("\n",iconv($l1['codepage'],"koi8-r",$n2));
$masstr=split("\n",html_entity_decode($n2,ENT_QUOTES,$l1['codepage']));
$i2=0;
foreach ($masstr as $t) { $i2++; if (strlen($t) >= $cols) $i2=$i2+(floor(strlen($t) / $cols)); }
...
if ($n == "email") { $tabl1='<font color=red>your e-mail:</font>'; }
